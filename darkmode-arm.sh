...
if [ "$ARCH" != "amd64" ] && [ "$ARCH" != "arm64" ]; then
    echo -e "${RED}Error: Arsitektur OS Anda (${ARCH}) tidak didukung. Skrip ini hanya untuk amd64 (PC/Server) dan arm64 (STB).${NC}"
    exit 1
fi
echo -e "${GREEN}Arsitektur terdeteksi: ${ARCH}. Melanjutkan instalasi...${NC}"
cd "$SCRIPT_DIR" || exit

setup_mongodb_docker() {
    if ! command -v docker >/dev/null 2>&1; then
        echo -e "${GREEN}Docker tidak ditemukan. Menginstall Docker... (Bisa memakan waktu)${NC}"
        sudo apt-get update -y || { echo -e "${RED}Gagal menjalankan apt update.${NC}"; return 1; }
        sudo apt-get install -y docker.io || { echo -e "${RED}Gagal menginstall docker.io. Cek error di atas.${NC}"; return 1; }
        if ! command -v docker >/dev/null 2>&1; then
            echo -e "${RED}Instalasi Docker selesai tapi command 'docker' tidak ditemukan. Gagal.${NC}"
            return 1
        fi
        echo -e "${GREEN}Docker berhasil diinstall.${NC}"
    fi

    echo -e "${GREEN}Memastikan service Docker berjalan dan menonaktifkan mongod lama...${NC}"
    ...
    for tag in "${docker_mongo_tags[@]}"; do
        if ! sudo docker ps -a --format '{{.Names}}' | grep -qx 'mongo-genieacs'; then
            echo -e "${GREEN}Menarik image Docker mongo:${tag}... (Ini bisa memakan waktu beberapa menit)${NC}"
            if ! sudo docker pull "mongo:${tag}"; then
                echo -e "${RED}Gagal menarik image Docker mongo:${tag}. Mencoba tag berikutnya...${NC}"
                continue
            fi
            echo -e "${GREEN}Menjalankan container Docker mongo-genieacs...${NC}"
            container_id=$(sudo docker run -d --log-driver=json-file --log-opt max-size=10m \
                --name mongo-genieacs \
            ...
            if [ $? -ne 0 ] || [ -z "$container_id" ]; then
                echo -e "${RED}Gagal menjalankan container Docker. Cek log Docker di bawah.${NC}"
                sudo docker logs mongo-genieacs --tail 50 || true
                sudo docker rm -f mongo-genieacs >/dev/null 2>&1 || true
                continue
            fi
        fi

        echo -e -n "${GREEN}Menunggu MongoDB (via Docker) siap ${NC}"
        for ((i = 1; i <= 40; i++)); do
            if command -v mongosh >/dev/null 2>&1; then
                mongosh "mongodb://127.0.0.1:27017/admin" --quiet --eval 'db.runCommand({ ping: 1 })' >/dev/null 2>&1 && echo && return 0
            elif command -v mongo >/dev/null 2>&1; then
                mongo --host 127.0.0.1 --port 27017 --quiet --eval 'db.runCommand({ ping: 1 })' >/dev/null 2>&1 && echo && return 0
            else
                echo && return 0 # No mongo client to test, assume it's ok
            fi
            echo -n "."
            sleep 1.5
        done
...
ensure_mongodb_running() {
    ...
    echo -e -n "${GREEN}Menunggu service mongod merespon ${NC}"
    for ((i = 1; i <= 40; i++)); do
